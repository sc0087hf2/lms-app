  ...
  <section class="text-gray-600 body-font relative">
      ...
      <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li aria-current="page">
          <div class="flex items-center">
            ...
            <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">英単語追加</span>
          </div>
        </li>
      </ol>
    </nav>
    <div class="container px-5 py-8 mx-auto">
      <div class="flex flex-col text-center w-full mb-12">
        <h1 class="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">英単語　追加</h1>
      </div>
      <div class="lg:w-1/2 md:w-2/3 mx-auto">
        <form method="POST" action="{{ route('admin.words.store') }}" class="flex flex-wrap -m-2">
          @csrf
          <!-- 品詞選択 -->
          <div class="p-2 mb-2 w-full">
            <div class="relative">
              ...
              <select id="part_of_speech_id" name="part_of_speech_id" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                <option value="" disabled {{ old('part_of_speech_id') === null ? 'selected' : '' }}>選択してください</option>
                @foreach($partOfSpeeches as $partOfSpeech)
                <option value="{{ $partOfSpeech->id }}" {{ old('part_of_speech_id') == $partOfSpeech->id ? 'selected' : '' }}>{{ $partOfSpeech->name }}</option>
                @endforeach
              </select>
            </div>
          ...
          <div class="p-2 mb-2 w-full">
            <div class="relative">
              ...
              <input type="text" id="en_word" name="en_word" value="{{ old('en_word') }}" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
            </div>
            @error('en_word')
            <div class="alert alert-danger text-red-700">{{ $message }}</div>
            @enderror
          ...
          <div class="p-2 mb-2 w-full">
            <div class="relative">
              ...
              <input type="text" id="ja_word" name="ja_word" value="{{ old('ja_word') }}" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
            </div>
            @error('ja_word')
            <div class="alert alert-danger text-red-700">{{ $message }}</div>
            @enderror
