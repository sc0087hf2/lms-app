      ...
      <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li aria-current="page">
          <div class="flex items-center">
            ...
            <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">英文法一覧</span>
          </div>
        </li>
      </ol>
    </nav>
    <div class="container px-5 py-8 mx-auto">
      <div class="flex items-center justify-between mb-8 mx-5">
        <h1 class="sm:text-4xl text-3xl font-medium title-font text-gray-900">
          <a href="{{ route('admin.grammars.index') }}">
            英文法一覧
          </a>
        </h1>
        <button
          onclick="location.href='{{ route('admin.grammars.create') }}'"
          class="text-white bg-yellow-500 border-0 py-2 px-6 focus:outline-none hover:bg-yellow-600 rounded">
          <span class="hidden md:inline">英文法の</span>追加
        </button>
      </div>
      <!-- 検索フォーム -->
      <div class="relative w-64 mx-auto mb-4">
        <form action="{{ route('admin.grammars.index') }}" method="GET" class="flex">
          <input
            type="text"
            name="search"
      ...
      <div class="w-full mx-auto">
        <table class="table-auto text-left whitespace-no-wrap">
          <thead>
            <tr>
              <th class="border-b-4 border-blue-900 px-4 py-3 w-64 sm:w-96 title-font tracking-wider text-black text-sm">名前</th>
          ...
          <tbody>
            ...
            <tr>
              <td class="border-b-2 border-dashed border-gray-200 px-4 py-6">{{ $grammar->name }}</td>
              <td class="border-b-2 border-dashed border-gray-200 px-4 py-6">
                <a href="{{ route('admin.grammars.edit', ['grammarId' => $grammar->id]) }}">
              ...
              <td class="border-b-2 border-dashed border-gray-200 px-6">
                <form
                  action="{{ route('admin.grammars.destroy', ['grammarId' => $grammar->id]) }}"
                  method="POST"
      ...
      <!-- ページネーション -->
      <div class="py-6">
        {{ $grammars->links('pagination::default') }}
      </div>
    </div>
  </section>
</x-admin-layout>