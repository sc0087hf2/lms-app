<x-student-layout>
  ...
  <nav class="flex mt-2 mb-4 sm:mb-8" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
      ...
      <x-breadcrumb-list-last name="取り組み振り返り追加" />
    </ol>
  </nav>
  <section class="text-gray-600 body-font relative sm:w-192">
    <div class="container px-5 py-8 mx-auto">
      <div class="flex flex-col text-center w-full mb-4">
        <h1 class="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">取り組み振り返り　追加</h1>
      </div>
      <div class="w-full sm:w-4/5 mx-auto">
        <form method="POST" action="{{ route('student.comments.storeGoalComment', ['goalId' => $goal->id]) }}" class="flex flex-wrap -m-2">
          @csrf
          <!-- フィードバック -->
