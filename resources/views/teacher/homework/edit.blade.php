  ...
  <section class="text-gray-600 body-font relative">
      ...
      <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li aria-current="page">
          <div class="flex items-center">
            ...
            <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">宿題編集</span>
          </div>
        </li>
      </ol>
    </nav>
    <div class="container px-5 py-8 mx-auto">
      <div class="flex flex-col text-center w-full mb-12">
        <h1 class="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">宿題　編集</h1>
      </div>
      <div class="lg:w-1/2 md:w-2/3 mx-auto">
        <form method="POST" action="{{ route('teacher.homework.update', ['homeworkId' => $homework->id]) }}" class="flex flex-wrap -m-2">
          @csrf
          <!-- ToDo -->
          <div class="p-2 mb-2 w-full">
            <div class="relative">
              <label for="todo_id" class="leading-7 text-sm text-gray-600">ToDoの選択（必須）</label>
              <select id="todo_id" name="todo_id" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                <option value="" disabled {{ old('todo_id', $homework->todo_id) === null ? 'selected' : '' }}>選択してください</option>
                @foreach($todos as $todo)
                <option value="{{ $todo->id }}" {{ old('todo_id', $homework->todo_id) == $todo->id ? 'selected' : '' }}>{{ $todo->todo }}</option>
                @endforeach
              </select>
            </div>
            @error('todo_id')
