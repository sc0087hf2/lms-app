  ...
  <section class="text-gray-600 body-font relative">
      ...
      <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li aria-current="page">
          <div class="flex items-center">
            ...
            <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">ToDo追加</span>
          </div>
        </li>
      </ol>
    </nav>
    <div class="container px-5 py-8 mx-auto">
      <div class="flex flex-col text-center w-full mb-4">
        <h1 class="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">ToDo　追加</h1>
      </div>
      <div class="lg:w-1/2 md:w-2/3 mx-auto">
        <form method="POST" action="{{ route('teacher.todos.store') }}" class="flex flex-wrap -m-2">
          ...
          <div class="p-2 mb-2 w-full">
            <div class="relative">
              <label for="goal_id" class="leading-7 text-sm text-gray-600">目標を選択してください（必須）</label>
              <select id="goal_id" name="goal_id" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                <option value="" disabled {{ old('goal_id') === null ? 'selected' : '' }}>選択してください</option>
                @foreach($goals as $goal)
                <option value="{{ $goal->id }}" {{ old('goal_id') == $goal->id ? 'selected' : '' }}>{{ $goal->goal }}</option>
                @endforeach
              </select>
            </div>
            @error('goal_id')
